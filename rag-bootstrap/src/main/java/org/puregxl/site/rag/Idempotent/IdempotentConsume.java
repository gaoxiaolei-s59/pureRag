package org.puregxl.site.rag.Idempotent;

import java.lang.annotation.ElementType;
import java.lang.annotation.Retention;
import java.lang.annotation.RetentionPolicy;
import java.lang.annotation.Target;

/**
 * 幂等注解
 */
@Target(ElementType.METHOD)
@Retention(RetentionPolicy.RUNTIME)
public @interface IdempotentConsume {

    /**
     * 幂等业务 Key，支持 SpEL，例如：#message.uuid。
     * 为空时会尝试从第一个 MessageWrapper 参数中读取 uuid，uuid 为空再读取 keys。
     */
    String key() default "";

    /**
     * 重复提交或重复消费时的提示信息。
     */
    String message() default "请求正在处理中，请稍后再试";

    /**
     * 幂等 Key 过期时间，单位秒。消费场景需要覆盖消息最大重试窗口，避免业务成功后重复投递再次执行。
     */
    long expireSeconds() default 7200L;

    /**
     * 重复命中时是否抛出异常。MQ 消费默认直接忽略重复消息；接口防重复提交可设置为 true。
     */
    boolean throwOnRepeat() default false;

}
