...
public class MutiQueryRewriteService implements QueryRewriteService {
    ...
    private static final String PROMPT_RESOURCE_PATH = "prompt/query-rewrite-prompt.txt";
    ...
    public String rewriteRequestBuilder(String userQuestion, List<ChatMessage> history) {
        return promptTemplateLoader.load(PROMPT_RESOURCE_PATH)
                .replace("{{history}}", buildHistoryText(history))
                .replace("{{question}}", userQuestion.trim());
    }
